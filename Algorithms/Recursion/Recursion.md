...
- [📊 Analysis of Recursion](#-analysis-of-recursion)
  - [⏱️ Time Complexity](#️-time-complexity)
    ...
    - [Master Theorem Method](#master-theorem-method)
...
Analysing the recursion involves considering a two key aspects:
1. Time Complexity — how the time requirements grow with the input size.
...
1. **Substitution** — guess a bound, than prove with mathematical induction. Beneficial when you have a good guess.
2. **Recursion Tree** — visualize the recurrence as a tree and calculating nodes sum per level together. Used for generating good guesses for substitution method and also provides proof for master method.
3. **Master Theorem** — provides solution for recurrences of the particular form. Used when the recurrence in a specific form, which pretty frequently occurs in many commonly encountered algorithms.

Analyzing and solving recurrences is a nuanced process, and the choice of method depends on the characteristics of the recurrence relation. Each method has its strengths, making it well-suited for certain types of recurrences. Detailed overview of each of these methods you can find below.

---

**1. Substitution Method** — guess a bound, than prove with mathematical induction. Method is powerful, but in order to apply there must be good guess of a form and there is no general way to do so, it takes experience and creativity, although heuristics and recursion trees can help generate those guesses.

Currently in progress...

---

**2. Recursion Tree Method**  — visualize the recurrence as a tree, where each node represents the cost of a single subproblem, those costs on each tree level form a per-level costs, and the sum of those per-level costs is the overall complexity.

Currently in progress...

---

**3. Master Theorem Method** — provides solution for recurrences of the form $T(n) = aT(\frac{n}{b}) + f(n)$, where:
...
- [Рекуррентные уравнения](https://youtu.be/NESmtDBA6Cw?si=Mr9Pl_mPC98pp9WZ) (Lecture)
